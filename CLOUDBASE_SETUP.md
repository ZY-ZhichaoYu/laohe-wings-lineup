...
这个函数会把当前阵容写入 `lineups/current`，也会处理朋友评价、评价回复、评价/回复撤回，并在手机端导出 PNG 时代理读取云存储图片，避免 iOS/微信浏览器因为跨域图片阻止 `canvas` 导出。因为 `lineups` 是 `ADMINWRITE`，普通网页用户只能读，不能直接改数据库；写入统一走这个云函数。
